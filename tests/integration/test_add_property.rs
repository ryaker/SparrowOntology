...
fn add_property_to_existing_class() {
    ...
    let prop = add_property(&db, "Person", "age", "int64", false).unwrap();
...
fn add_required_property() {
    let (_dir, db) = initialized_db();
    let prop = add_property(&db, "Task", "deadline", "date", true).unwrap();
    assert!(prop.required);
}
...
fn add_property_variant_datatype() {
    let (_dir, db) = initialized_db();
    let prop = add_property(&db, "Concept", "metadata", "variant", false).unwrap();
...
fn add_property_unknown_class_returns_error() {
    let (_dir, db) = initialized_db();
    let err = add_property(&db, "NonExistentClass", "foo", "string", false).unwrap_err();
    assert!(
...
fn add_property_reserved_name_returns_error() {
    let (_dir, db) = initialized_db();
    let err = add_property(&db, "Person", "__so_secret", "string", false).unwrap_err();
    assert!(
...
fn add_property_duplicate_returns_error() {
    let (_dir, db) = initialized_db();
    add_property(&db, "Person", "nickname", "string", false).unwrap();
    let err = add_property(&db, "Person", "nickname", "string", false).unwrap_err();
    assert!(
...
fn add_property_validates_entity_with_new_prop() {
    let (_dir, db) = initialized_db();
    add_property(&db, "Task", "due_date", "date", false).unwrap();
...
fn add_property_via_alias_resolves_owner() {
    ...
    let prop = add_property(&db, "Org", "industry", "string", false).unwrap();
